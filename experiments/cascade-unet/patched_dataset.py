...
class PatchDataset(data.Dataset):
    def __init__(self, downsampled_dataset: DownsampledDataset, patch_size=16):
        super().__init__()
        self.dataset = downsampled_dataset.dataset
        self.downsampled_dataset = downsampled_dataset
    ...
    def __getitem__(self, index):
        ...
        if idx == self.last_data_idx:
            ...
            _, seg_down = self.downsampled_dataset[idx]
            scan = torch.concat([scan, self.downsampled_dataset.up(seg_down.unsqueeze(0))[0]], dim=0)
            del self.last_data
...
if __name__ == '__main__':
    ...
    down_dataset = DownsampledDataset(dataset)
    patch_dataset = PatchDataset(down_dataset)
    patches = [], []
